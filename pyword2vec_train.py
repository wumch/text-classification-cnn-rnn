...
class Train:
    ...
    def __init__(self, extra_dict: str=None):
    ...
    def _seg(self):
        ...
        with open(self.input_file) as ifp, open(self.seged_file, 'w') as ofp:
            for line in ifp:
                ofp.write(' '.join(self._seg_line(line)) + os.linesep)
        print(f'seged file: {self.seged_file}')

    def _seg_line(self, text: str):
...
if __name__ == '__main__':
    _train = Train(extra_dict='../pubu/config/data/extra-dict.txt')
    _train.run()
