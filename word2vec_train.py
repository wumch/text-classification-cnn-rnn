...
def read_data():
    ...
    spaces = re.compile('\s+')
    raw_word_list = []
    for line in open('/dev/shm/doupocangqiong.txt',"r", encoding='UTF-8'):
        line = line.replace('　', ' ')  # 空格 全角转半角
...
def build_dataset(words):
    ...
    for word in words:
        ...
        data.append(index)
    count[0][1] = unk_count
    reverse_idx_map = dict(zip(idx_map.values(), idx_map.keys()))
    return data, count, idx_map, reverse_idx_map
